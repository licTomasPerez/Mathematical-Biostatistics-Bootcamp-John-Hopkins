...
%In statistical inference it is often the case that the fitted model is not correct. Thus it is necessary to analyse the performance of statistical methods if the underlying model is not correct, looking for some robust alternatives to least squares estimation. \\

%Let $\xdata$ be an observation, then the mean and median can be defined as the minimizers of the following quantities 

%\begin{align}    \textnormal{mean }& : \min_{m \in \Theta} \summation (x_i - m)^2, &   \textnormal{median }& : \min_{m \in \Theta} \summation |(x_i - m)|.\end{align}

%For simple linear regression, observing $\dataset$, the least squares satisfy 

%\begin{align*}    \textnormal{least squares }: \min_{\substack{a \in \Theta \\                      b \in \Theta }} \summation |y_i - (\alpha + \beta x_i)|^2.\end{align*}

%Analogously, the least absolute deviation regression estimates are given by 

%\begin{equation}    \textnormal{least absolute deviation }: \min_{\substack{a \in \Theta \\                     b \in \Theta }} \summation |y_i - (\alpha + \beta x_i)|.\end{equation}
