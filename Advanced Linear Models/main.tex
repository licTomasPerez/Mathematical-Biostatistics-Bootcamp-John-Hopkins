...
It can be calculated that $\E \hat{\epsilon}_i = 0$ \footnote{In effect, using the residuals from the regression's definition we have 
...
\begin{theo}\footnote{\begin{proof}
...
\begin{align*}
    ...
    &= \alpha + {\beta \bar{x}} - \frac{\beta \bar{x}}{S_{xx}} \underbrace{\summation(x_i - \bar x)^2}_{S_{xx}} = \alpha  \textnormal{ with the variance being }
    \var \hat \alpha = \sigma^2 \summation c_i^2 = \frac{\sigma^2}{nSx_{xx}} \summation x_i^2
\end{align*}
