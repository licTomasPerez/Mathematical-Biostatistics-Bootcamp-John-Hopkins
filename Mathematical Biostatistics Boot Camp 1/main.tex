...
\section{Quiz 4 - Week 4}

\begin{tcolorbox}[title=Question 1]
You would like to estimate the prevalence of a disease. You randomly sample 10 people from the population of interest and 4 of them have the condition. Using a Beta prior with $\alpha$ and $\beta$ both being 10, what is the posterior mean prevalence closest to?
\end{tcolorbox}

\begin{tcolorbox}[title=Question 2]
You simulate a lot of lognormal(5, 1) random variables and take their geometric mean. What will it likely be close to?
\end{tcolorbox}
...
\begin{tcolorbox}[title=Question 4]
If you simulate a very large number of binomial(n,p) random variables, calculate a 95\% Wald confidence interval for p and calculate the percentage of times the intervals covers the value of p used for simulation. p is neither 0 or 1. Which of the following are true about the percentage of times the intervals cover the true value (check all that are true)?
\end{tcolorbox}

\begin{tcolorbox}[title=Question 5]
The difference between $\mu$ and $\bar x$ is
\end{tcolorbox}
\end{document}
