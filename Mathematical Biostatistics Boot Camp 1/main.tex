...
\begin{tcolorbox}[title=Question 8]
Consider any distribution function FF with associated mean 00 and variance 11. Notice that if $X$ is a random variable from FF, then $Y = \mu + \sigma X$ has mean $\mu$ and variance $\sigma^2$. What is the distribution function associated with $Y$? (Call it $G$.)
\end{tcolorbox}
